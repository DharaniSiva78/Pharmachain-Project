...
document.getElementById("spoilBtn").addEventListener("click", async () => {
  const batchId = document.getElementById("spoilBatchId").value.trim();
  const reason = document.getElementById("spoilReason").value.trim();
  ...
  try {
    showResult("⏳ Checking batch status...", "loading");
    ...
    if (isExpired) {
      showResult("❌ Batch is already expired.", "error");
      return;
    }

    if (batchDetails[7]) { // already spoiled
      showResult("❌ Batch is already marked as spoiled.", "error");
      return;
    }

    showResult("⏳ Marking batch as spoiled...", "loading");
    
    const tx = await contract.markAsSpoiled(batchId, reason);
    showResult("⏳ Transaction submitted. Waiting for confirmation...", "loading");
    
    await tx.wait();
    showResult("⚠️ Batch marked as spoiled successfully!", "success");
    
    // Clear form
    document.getElementById("spoilBatchId").value = "";
    document.getElementById("spoilReason").value = "";
    
  } catch (err) {
    console.error("Spoil error:", err);
    let errorMessage = "❌ Error marking batch as spoiled.";
